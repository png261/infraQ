...
export function CursorDrivenParticleTypography({
    ...
    useEffect(() => {
        ...
        const init = () => {
            ...
            const horizontalPadding = Math.max(24, containerWidth * 0.04);
            const maxTextWidth = Math.max(1, containerWidth - horizontalPadding * 2);
            const maxTextHeight = Math.max(1, containerHeight * 0.72);
            let effectiveFontSize = Math.min(fontSize, maxTextHeight);
            ctx.font = `bold ${effectiveFontSize}px ${fontFamily}`;
            const measuredWidth = ctx.measureText(text).width;
            if (measuredWidth > maxTextWidth) {
                effectiveFontSize *= maxTextWidth / measuredWidth;
            }
            ctx.font = `bold ${effectiveFontSize}px ${fontFamily}`;
    ...
    return (
        <div
            ref={containerRef}
            className={cn("w-full h-full min-h-0 flex items-center justify-center relative touch-none", className)}
        >
            <canvas
                ref={canvasRef}
